...
dwim = (cmd-button "$toggle_cmd runner")
[[public]]
...
#+begin_src sh
if [[ ("$2" != "runner") && ("$2" != "terminal") ]]; then
    echo "Invalid second argument: should be runner|terminal" >&2
    exit 1
fi
#+end_src
...
processAction() {
    case $1 in
        focus)
            ...
            if wmctrl -x -a $target_classname; then
        ...
        toggle)
            ...
            if wmctrl -x -c $target_classname; then
                # successfully closed!
                exit 0
            fi
            ;;
        dwim)
            ...
            if xprop -notype -id "$focused_window" WM_CLASS | grep -q "$target_classname"; then
                # close window
            ...
            ;;
        ,*)
            echo "Invalid first argument: please pass in focus|toggle|dwim" >&2
            exit 1
    esac
}
...
if [[ "$2" == "runner" ]]; then
    ...
    printf -v "TERMINAL_WIDTH" "%.0f" $(echo "0.369792 * $WIDTH + 750" | bc)
    printf -v "TERMINAL_HEIGHT" "%.0f" $(echo "0.76389 * $HEIGHT - 70" | bc)
    ...
    printf -v "TERMINAL_ROWS" "%.0f" $(echo "scale=3; $TERMINAL_HEIGHT / 36.0" | bc)
    # measured: 14.26 pixels per column
    printf -v "TERMINAL_COLS" "%.0f" $(echo "scale=3; $TERMINAL_WIDTH / 14.26" | bc)
    y_padding=$(((HEIGHT - TERMINAL_HEIGHT) / 2))
fi
...
st_command=()
if [[ "$2" = "runner" ]]; then
    st_args=(-m 15 -c "rounded $target_classname")
    st_command=(screen -c ~/.config/kmonad/runner/screenrc -DR runner)
else
    st_args=(-c "$target_classname")
    st_command=(tmux new -As kmonad-terminal)
fi
#+end_src
...
#+begin_src sh
|>ST<| "${st_args[@]}" -g "${TERMINAL_COLS}x${TERMINAL_ROWS}+${terminal_x_offset}+${terminal_y_offset}" "${st_command[@]}"
#+end_src
