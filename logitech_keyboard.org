...
sub addJumpEntry {
    $activeEntryNum += 1;
    @currentJumpList[$activeEntryNum] = "$currentDesktop $focusedWindow";
...
sub shiftStackPointer {
    ...
    my ($desktop, $window) = split / /, $currentJumpList[$activeEntryNum];
...
given ($ARGV[0]) {
    when (m/add/) { addJumpEntry }
    when (m/next/) { nextEntry }
    when (m/prev/) { prevEntry }
    default {
        print 'Please specify `add`, `next`, or `prev` as the first argument.';
        exit 1;
    }
}
