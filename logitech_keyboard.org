...
#+begin_src kbdx
[desktop-window]
parent = { leader, numeric-desktop-switching, alphabetic-window-tiling }
#+end_src
...
I want to modify the ~a~ key behavior so that ~a+Enter~ hits enter and then closes the terminal.
#+begin_src kbdx
[[keys]]
a = (tap-hold $hmod-delay a (layer-toggle run.home-row-chord))
lmet = exit
#+end_src
** Runner Home Row Chord Layer
#+begin_src kbdx
[[home-row-chord]]
parent = home-row-chord
[[[keys]]]
return = #('enter' @run:exit)
#+end_src
** Runner Script
:PROPERTIES:
