...
*** Tap
- On accepting a window, switches to it
*** Hold
- On accepting a window, pulls it to the current desktop
** D :: Discord
Opens Discord.
** E :: Emacs
Opens Emacs.
*** Tap
Opens an Emacs client instance.
*** Hold
Opens a new Emacs standalone instance.
** F :: Firefox
Opens Firefox
** B :: Brightness
Initiates ~BrightnessControl~ layer, where we can manipulate brightness with ~hjkl~.
...
** R :: Run
Intended to run commands in a small terminal window.
Appears at the top of the screen, and expands down as the output fills.
*** Layer
**** TODO E
Make window taller.
**** Super
Exit layer.
**** a+Enter
Hits enter and then exits the layer.
*** Notes
**** Keybindings
...
Opens agenda.
** O :: Open Preset
Just opens a ~rofi~ selection menu for selecting between different windowing presets.
...
***** TODO Preset-Local Shortcuts
****** O :: Open Problem List
Opens the ~emacs~ window once again and loops from step #1.
** C :: Org-Capure
Opens org-capture.
I already have this bound to ~Super+C~, but having it on ~Super, C~ makes it easier to invoke.
** S :: Scroll
...
** Y :: Yank
Copies the focused window's title to the clipboard.
** TODO \ :: Local Leader Key
Use this in conjunction with presets to have local preset keybindings.
** Tab :: Switch Focus
When tapped, switches the focus between monitors.
When held for more than a certain duration, moves the cursor over to the next monitor.
** Super :: Super
...
focus = (cmd-button "$runner_script focus")
toggle = (cmd-button "$runner_script toggle")
[[public]]
entrypoint = (tap-hold-next-release $tap-hold-delay @focus (layer-toggle run))
[[keys]]
t = toggle
#+end_src
** Runner Script
...
Let's parse the command-line to figure out what we should do.
...
On the other hand, if we want to ~toggle~, then let's attempt to _close_ a window containing the ~st-runner~ class, and if /that/ fails, then we leverage the same code as ~focus~ for creating the new instance!
#+begin_src sh
case $1 in
    focus)
    ...
    ;;
    ,*)
        echo "Invalid argument: please pass in focus|toggle" >&2
        exit 1
esac
#+end_src
