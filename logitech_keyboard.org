...
Using ~tmux~ to maintain a single shell session through each ~runner~ invocation.
I was initially using ~screen~, but ~screen~ is extremely broken on Arch: when I run the following commands, it results in a ~screen caught signal 11~ error from the following sequence of commands
1. ~st /bin/bash -c 'screen -S runner'~
2. ~killall st~
3. ~screen -r runner~


- ~-f ...~ ::
  Uses the corresponding configuration file.
- ~new-session -At runner~ ::
  Attaches to a session called ~runner~, creating it if necessary.

#+begin_src sh
|>ST<| -c "rounded st-runner" -g "116x8+${terminal_x_offset}+${terminal_y_offset}" tmux -f ~/.config/kmonad/runner/tmux.conf new-session -At runner
#+end_src
...
#+begin_src sh
tmux send -t runner.0 "C-l"
#+end_src

This is the configuration we are using for ~tmux~.
We disable the statusline and also set the default shell to Fish.
#+begin_src text :tangle ~/.config/kmonad/runner/tmux.conf
set -g status off
