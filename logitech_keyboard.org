...
(defalias
  ...
  enter-volume #((cmd-button "echo 1 > /tmp/kmonad_volume_script_display_osd") (layer-add volume))
  exit-volume (layer-rem volume)

  volume-up   (cmd-button "~/.config/kmonad/volume/volume.sh +"
                          "~/.config/kmonad/volume/volume.sh 0")
  volume-down (cmd-button "~/.config/kmonad/volume/volume.sh -"
                          "~/.config/kmonad/volume/volume.sh 0")

  volume-osd-toggle (cmd-button "~/.config/kmonad/volume/volume_popup_toggle.sh")
...
#+begin_src sh
DIRECTION_PID_FILE=/tmp/kmonad_volume_script
...
#+begin_src sh
    DISPLAY_OSD_FILE=/tmp/kmonad_volume_script_display_osd
...
#+begin_src sh
    exec python2 -ES ~/.config/kmonad/volume/change_volume.py $CURRENT_VOLUME $VOLUME_CHANGE_DIRECTION $DISPLAY_OSD
fi
#+end_src
...
if [ -e /tmp/kmonad_volume_script ]; then
    kill -s USR1 $(cat /tmp/kmonad_volume_script)
fi
#+end_src
