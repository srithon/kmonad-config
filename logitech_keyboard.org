...
Exit layer.
** V :: Volume
Layer that allows you to easily change the volume.
*** Volume Layer
**** H
...
#+begin_src sh
    exec python2 -ES ~/.config/kmonad/volume/change_volume.py $CURRENT_VOLUME $VOLUME_CHANGE_DIRECTION
fi
#+end_src
...
#+begin_src python :tangle ~/.config/kmonad/volume/change_volume.py
